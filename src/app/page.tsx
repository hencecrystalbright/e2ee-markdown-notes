...
function NoteApp() {
  ...
  const [isEmojiSectionOpen, setIsEmojiSectionOpen] = useState(false); // 控制 Icon 面板開關
  ...
  const handleInsertImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // 進行圖片等比壓縮（防呆容錯，若 Canvas 失敗則退回原圖）
      ...
      // ✅ 換成 ImgBB 專屬 API Key（免驗證、穩定不擋 IP）
      const IMGBB_API_KEY = "16ea7f74ac471604d9c319efef2cda0e";
      ...
      if (data.success && data.data?.url) {
        ...
        alert(`圖片上傳失敗 (${response.status}): ${data.error?.message || '請確認網路或 API Key 狀態'}`);
      }
    } catch (error) {
      console.error("圖片壓縮/上傳失敗:", error);
      alert("圖片處理異常，請檢查網路連線。");
    }
